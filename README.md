# USX Documents (2024.6)

This repository contains a Markdown snapshot of the **USX - 2024.6** documentation, plus the local image assets referenced by the doc.

## Contents

- `docs.md` — the main documentation (overview, FAQs, price stability, minting/redeeming modules, liquidity modules, etc.)
- `USX - 2024 6/` — images referenced from `docs.md`

## Read

- Open `docs.md` directly in GitHub, or in any Markdown editor (VS Code, Typora, Obsidian, etc.).
- Images referenced like `USX%20-%202024%206/Untitled.png` should render correctly as long as the folder name and filenames are unchanged.

## Editing / Updating

- Keep asset paths stable (the doc currently references images using URL-encoded spaces: `USX%20-%202024%206/...`).
- If you rename/move images, update the corresponding links in `docs.md`.

## Contributing

1. Create a branch
2. Make changes to `docs.md` and/or images under `USX - 2024 6/`
3. Open a PR with a short summary of what changed and why

## Notes

- This repo is documentation-only (no build/test steps).
- A license file is not included yet; add one if you intend to open-source or redistribute these docs.

